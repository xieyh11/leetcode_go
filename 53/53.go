...
func recursiveSol(nums []int, left, right bool, currentSum int) int {
	if len(nums) <= 1 {
		return currentSum
	}
	il := 0
	...
	leftSum := 0
	rightSum := 0
	for ; left && il < len(nums)-1; il += 2 {
		leftSum += nums[il] + nums[il+1]
		if leftSum < 0 {
			break
		}
	}
	...
	if left && right {
		if il < len(nums) {
			if jr >= 0 {
				if il <= jr {
					tempL := recursiveSol(nums[:il+1], false, true, leftSum+nums[il])
					tempR := recursiveSol(nums[jr:], true, false, rightSum+nums[jr])
					return max(tempL, max(tempR, recursiveSol(nums[il:jr+1], true, true, currentSum-leftSum-rightSum)))
				} else {
					leftR := recursiveSol(nums[:jr+1], false, true, currentSum-rightSum)
					rightR := recursiveSol(nums[il:], true, false, currentSum-leftSum)
					return max(leftR, rightR)
				}
			} else {
				return recursiveSol(nums[il:], true, false, currentSum-leftSum)
			}
		} else {
			if jr >= 0 {
				return recursiveSol(nums[:jr+1], false, true, currentSum-rightSum)
			}
		}
	} else if left {
		if il < len(nums) {
